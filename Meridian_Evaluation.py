...
# np.savetxt('Data/SECS_Running_Files/Ultimate_Matrix.txt', Output)


# # for date in pd.date_range(start=  pd.Timestamp('1999-01-01T00:00:00'), end= pd.Timestamp('2019-12-31T23:59:59'), freq='min'):                                                                                                                                         
# start=time.time()
# for date in pd.date_range(start=  pd.Timestamp('1999-01-01T00:00:00'), end= pd.Timestamp('1999-01-01T00:03:00'), freq='min'):
#     date=pd.Timestamp('2000-10-03T02:10:00')
#     date=np.datetime64(date)
#     # date_index= store.index==date
#     # date_index= store.index==date
#     # index= site_index & date_index
#     # store.select(index)
#     # Btheta, Bphi, Br=store.evaluate(store.Btheta, selection=True), store.evaluate(store.Bphi, selection=True), store.evaluate(store.Br, selection=True)
#     # index= store.index==date
#     DataFrame= store[store.index==date].to_pandas_df
#     index= DataFrame['Site'].isin(sites)
#     DataFrame= DataFrame[index]
#     Btheta, Bphi, Br= -np.unique(DataFrame.Btheta.values)*1e-9,np.unique(DataFrame.Bphi.values)*1e-9,-np.unique(DataFrame.Br.values)*1e-9
#     if len(Btheta)!=len(sites):
#         continue
#     else:
#         Currents= poles.Currents(Btheta, Bphi, Br, singularity_limit= 50e3)
#         Magnetic_Field=poles.Magnetic_Field(Btheta, Bphi, Br)
# #         break
# end=time.time()
# print('vaex', end-start)
# start=time.time()
start_time=time.time()
Ultimate_Matrix= np.loadtxt('Data/SECS_Running_Files/Ultimate_Matrix.txt')
# # HDF_path= '/Home/siv32/zef014/Documents/Masters_Thesis_Code/Masters_Thesis/Data/batches_combined.hdf5'
epoch = 2010.
...
for date in pd.date_range(start=  pd.Timestamp('2016-01-01T00:00:00'), end= pd.Timestamp('2020-01-01T00:00:00'), freq='Y'):
#   for date in pd.date_range(start=  pd.Timestamp('2018-04-25T12:33:00'), end= pd.Timestamp('2018-04-25T12:58:00'), freq='D'):
    
    start= date
    end= date+ pd.DateOffset(years=1)
    #start= pd.Timestamp('2018-04-25T12:33:00')
    #end= pd.Timestamp('2018-04-25T12:58:00')
    ...
    index= DataFrame['Site'].isin(sites)
    DataFrame=DataFrame[index]
    # date_index=DataFrame.index==date
    dates=[]
    i=0
    ...
    if len(dates)==0:
        ...
        tmp_df.to_hdf('/Home/siv32/zef014/Documents/Masters_Thesis_Code/Masters_Thesis/Data/Meridian_Analysis/results.hdf5','main',mode='a',append=True,format='t', data_columns=True)
...
# end=time.time()
# tmp_df.to_csv('Data/test_run2.csv')
