...
thetap= np.deg2rad(np.linspace(0, 20))
# for rm in np.linspace(RE, RE+100e3, 10):
ax.plot(Rad2Deg(thetap), BrDF(thetap, rm, RI), label=f'RE + {rm-RE}')
# ax.plot(Rad2Deg(thetap), new_BrDF(thetap, rm, RI, np.deg2rad(10)), label='new')
ax.set_title('Br')
ax.set_xlabel("theta'")
ax.set_ylabel('Br (T)/ I0')
ax.legend(loc='best')
ax2.plot(Rad2Deg(thetap), BthetaDF(thetap, rm, RI), label=f'RE + {rm-RE}')
